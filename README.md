...
1. **Clone o repositório:**
   ```bash
   git clone [https://github.com/Gabriel-H182007/fiap-cpad-cp2-fiaproom-app.git](https://github.com/Gabriel-H182007/fiap-cpad-cp2-fiaproom-app.git)
   ```
2. **Acesse a pasta do projeto**
   ```bash
   cd fiap-cpad-cp2-fiaproom-app
   ```
...
## Demonstração
![Criação de conta](./assets/criacaoConta.gif)
![Tela de login](./assets/telaLogin.gif)
![Reserva de sala](./assets/reservaSala.gif)
...
* **Context API**: Foi adotado o uso de estados globais através de contextos (`AuthContext` e `AppDataContext`) para compartilhar os dados do usuário e das salas reservadas por todo o aplicativo, garantindo que as rotas autenticadas fiquem protegidas.
* **AsyncStorage**: Utilizado para persistir os dados do aplicativo localmente. Assim, as sessões de login e as reservas sobrevivem ao fechamento do aplicativo, mantendo a experiência do usuário fluida.

## Hooks Utilizados
* **useState**: Utilizado para gerenciar estados locais das telas, como dados exibidos e interações do usuário.
* **useEffect**: Utilizado para executar efeitos colaterais, como carregamento inicial de dados (leitura do AsyncStorage) ao abrir a tela.
* **useContext**: Utilizado para consumir os dados e funções fornecidos globalmente pelos nossos Providers (Context API).
...
`Notifications.scheduleNotificationAsync()` para gerenciar tanto notificações instantâneas quanto futuras.
...
* **Adicionar mais telas**: Criar novas funcionalidades ou páginas para deixar o app mais completo.
* **Melhorar navegação**: Ajustar botões e fluxo entre telas para ficar mais intuitivo.
```